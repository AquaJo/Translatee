...
import Binary from "./BinaryOperations";
...
export async function translate(kind: "received" | "sent", text: string): Promise<TranslationValue> {
    let translate = IS_WEB || settings.store.service === "google"
        ? googleTranslate
        : deeplTranslate;
    ...
    try {
        ...
        if (Binary.isBinary(text) || sourceLang === "binary" || targetLang === "binary") {
            translate = binaryTranslate;
        }
...
async function binaryTranslate(text: string, sourceLang: string, targetLang: string): Promise<TranslationValue> {
    const textIsBinary = Binary.isBinary(text);
    const targetIsBinary = targetLang === "binary";
    let finalSrcLang = sourceLang; // first guess, see next if statement for further guesses
    if (!textIsBinary && sourceLang === "auto") finalSrcLang = await googleTranslate(text, "auto", targetLang).then(obj => obj.sourceLanguage); // if auto and not binary use already inbuilt google translation function and extract source language recognized by google

    return {
        sourceLanguage: finalSrcLang,
        text: targetIsBinary ? (textIsBinary ? Binary.binaryToString(text) : Binary.stringToBinary(text)) : await googleTranslate(Binary.binaryToString(text), "auto", targetLang).then(x => x.text)
        // check cases: binary input and output --> original decoded output; binary input and not output --> original decoded output translated to targetLang; no binary input but output --> encode to binary directly
    };
}
